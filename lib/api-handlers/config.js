...
module.exports = async function handler(req, res) {
  ...
  const tagParam = typeof getQueryValue(req, "tag") === "string" ? getQueryValue(req, "tag").trim() : "";
  const tagGrantId = typeof getQueryValue(req, "tag_grant") === "string" ? getQueryValue(req, "tag_grant").trim() : "";
  ...
  const accountStats = await Promise.all(
    indexedAccounts.map(async (account) => {
      ...
      try {
        ...
        // authenticatedValidGrantCount = grants fonctionnels (200 sur /messages)
        // totalGrants = tous les grants Nylas (= slots occupes, determine si on peut en creer un nouveau)
        const grantsCount = metrics.totalGrants;
