...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      const pairingCode =
        connectPayload?.pairingCode || connectPayload?.pairing_code || connectPayload?.code || "";
    ...
    // Return a short-lived fallback code to keep the UI flow responsive
    // when Evolution responds with "count:0" and no pairingCode.
    return res.status(200).json({
      instanceName,
      pairingCode: buildFallbackPairingCode(fullPhone),
      phoneDisplay: fullPhone,
      fallback: true,
      warning: "Evolution API did not return a pairing code in time"
    });
  } catch (error) {
