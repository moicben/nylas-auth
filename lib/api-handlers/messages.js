...
module.exports = async function handler(req, res) {
  ...
  const subject = getQueryValue(req, "subject").trim();
  ...
  const hasSubjectSearch = Boolean(subject);
  params.set("limit", String(limit));
  if (hasSubjectSearch) {
    params.set("subject", subject);
  }
  if (!hasSubjectSearch && mailbox !== "OTHERS") {
  ...
  try {
    ...
    const withMailboxFilter =
      !hasSubjectSearch && mailbox === "OTHERS" ? list.filter(isOthersMailboxMessage) : list;
    const filteredData =
      readFilter === "all"
        ? withMailboxFilter
    ...
    return res.status(200).json({
      ...
      appliedFilters: {
        ...
        mailbox: hasSubjectSearch ? null : mailbox,
        globalSubjectSearch: hasSubjectSearch,
        read: readFilter
      }
    });
  } catch (error) {
