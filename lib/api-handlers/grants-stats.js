...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Compute stats in JS from the full dataset
    const total = allGrants.length;
    const valid = allGrants.filter(g => g.grant_status === "valid").length;
    const unauthorized = allGrants.filter(g => g.grant_status === "unauthorized").length;
    const done = allGrants.filter(g => g.grant_status === "done").length;
    const invalid = allGrants.filter(g => g.grant_status === "invalid").length;
    const deletedOnNylas = allGrants.filter(g => g.grant_status === "deleted_on_nylas").length;
    const softDeleted = allGrants.filter(g => g.deleted_at !== null).length;
    const uniqueEmails = new Set(allGrants.map(g => g.email).filter(Boolean)).size;
    const accounts = new Set(allGrants.map(g => g.account_id)).size;

    // Revoked count
    const revokedCount = allGrants.filter(g => g.revoked_at !== null).length;

    // Avg time to revoke (hours)
    const revokeDurations = allGrants
      .filter(g => g.revoked_at && g.nylas_created_at)
