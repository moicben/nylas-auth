...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Valid timeline : 3 derniers jours par tranches de 8h (9 buckets)
    // Un grant est "valide à l'instant T" si créé avant T et non supprimé/révoqué avant T
    const BUCKET_HOURS = 8;
    const BUCKET_MS = BUCKET_HOURS * 3600 * 1000;
    const NB_BUCKETS = 9;
    const now = Date.now();
    const alignedEnd = Math.ceil(now / BUCKET_MS) * BUCKET_MS;
    const validTimeline = [];
    for (let i = NB_BUCKETS - 1; i >= 0; i--) {
      const t = alignedEnd - i * BUCKET_MS;
      let count = 0;
      for (const g of effectiveGrants) {
        if (!g.nylas_created_at) continue;
        const created = new Date(g.nylas_created_at).getTime();
        if (created > t) continue;
        const deleted = g.deleted_at ? new Date(g.deleted_at).getTime() : null;
        if (deleted !== null && deleted <= t) continue;
        const revoked = g.revoked_at ? new Date(g.revoked_at).getTime() : null;
        if (revoked !== null && revoked <= t) continue;
        count++;
      }
      validTimeline.push({ ts: new Date(t).toISOString(), count });
    }
