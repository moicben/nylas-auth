...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const overviewResp = await fetch(
      `${baseUrl}/rest/v1/grants?select=id,grant_status,email,account_id,nylas_created_at,deleted_at`,
      { headers }
    );
    if (!overviewResp.ok) {
    ...
    for (const g of allGrants) {
      ...
      if (!dailyMap.has(day)) dailyMap.set(day, { day, total: 0, valid: 0, invalid: 0 });
