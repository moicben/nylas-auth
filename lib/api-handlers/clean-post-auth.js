...
function getMessageSenderEmails(message) {
  ...
  return message.from.map((entry) => normalizeEmail(entry?.email)).filter(Boolean);
}
    ...
    .map((folder) => {
      ...
      if (folder && typeof folder === "object") {
        const value =
          ...
          : typeof folder.display_name === "string" ? folder.display_name
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
      ...
      while (pages < DEFAULT_MAX_PAGES_PER_ATTEMPT) {
        const payload = await listMessages({
          apiUrl, apiKey, grantId, pageToken, limit: DEFAULT_PAGE_LIMIT
        });
        pages += 1;
      ...
      if (candidates.length) stats.foundOnAttempt = attempt;
      ...
      for (const message of candidates) {
        ...
        stats.matched += 1;
        try {
          await moveMessageToTrash({ apiUrl, apiKey, grantId, messageId });
          stats.movedToTrash += 1;
          try {
            await hardDeleteMessage({ apiUrl, apiKey, grantId, messageId });
            stats.hardDeleted += 1;
