...
const GRANTS_SELECT =
  "id,account_id,grant_id,provider,email,display_name,grant_status,is_valid,is_authenticated,nylas_created_at,last_checked_at,synced_at,deleted_at";
...
function grantToUpsertRow(grant, accountId, syncedAtIso, checkedAtIso) {
  ...
  const statusLower = normalizeGrantStatus(grantStatus);
  const isValid =
    typeof grant?.isValid === "boolean"
      ? grant.isValid
      : statusLower
        ? statusLower === "valid"
        : null;
  const isAuthenticated =
    typeof grant?.isAuthenticated === "boolean" ? grant.isAuthenticated : null;
...
async function softDeleteSupabaseGrant({ accountId, grantId, grantStatus = null }) {
  ...
  const normalizedStatus = sanitizeText(grantStatus);
  if (normalizedStatus) {
...
function mapSupabaseGrantForApi(row) {
  ...
  return {
    ...
    isAuthenticated: typeof row?.is_authenticated === "boolean" ? row.is_authenticated : null
  };
}
