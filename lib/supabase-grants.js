...
function ensureAccountId(value, label) {
...
async function listSupabaseGrantsByAccountId(accountId, { includeDeleted = false } = {}) {
  const safeAccountId = ensureAccountId(accountId, "accountId");
  const { baseUrl, authKey } = getSupabaseClientConfig();
  const filters = [
    `select=${encodeURIComponent(GRANTS_SELECT)}`,
    `account_id=eq.${encodeURIComponent(safeAccountId)}`,
...
async function softDeleteSupabaseGrant({ accountId, grantId, grantStatus = null }) {
  ...
  const response = await fetch(
    `${baseUrl}/rest/v1/grants?account_id=eq.${encodeURIComponent(safeAccountId)}&grant_id=eq.${encodeURIComponent(safeGrantId)}&deleted_at=is.null`,
    {
      method: "PATCH",
      headers: buildSupabaseHeaders(authKey, {
        "Content-Type": "application/json",
