...
const GRANTS_SELECT =
  "id,account_id,grant_id,provider,email,display_name,grant_status,nylas_created_at,last_checked_at,synced_at,deleted_at,revoked_at,tag";
...
function grantToUpsertRow(grant, accountId, syncedAtIso, checkedAtIso, tag) {
  ...
  const safeTag = tag ? sanitizeText(tag) : null;
  if (safeTag) {
    row.tag = safeTag;
  }
...
async function upsertSupabaseGrants({ accountId, grants, checkedAt, tag, tagGrantId }) {
  const safeAccountId = ensurePositiveInt(accountId, "accountId");
  ...
  const upsertRows = rows
    .map((grant) => {
      const grantTag = tagGrantId && sanitizeText(grant?.id || grant?.grantId) === tagGrantId ? tag : undefined;
      return grantToUpsertRow(grant, safeAccountId, syncedAtIso, checkedAtIso, grantTag);
    })
    .filter(Boolean);
