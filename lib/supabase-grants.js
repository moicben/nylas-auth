...
const GRANTS_SELECT =
  "id,account_id,grant_id,provider,email,display_name,grant_status,have_link,nylas_created_at,last_checked_at,synced_at,deleted_at,revoked_at,tag";
...
function mapSupabaseGrantForApi(row) {
  ...
  return {
    ...
    tag: sanitizeText(row?.tag) || null
  };
}

async function stampRevokedAt({ grantId, revokedAt }) {
  const safeGrantId = sanitizeText(grantId);
  if (!safeGrantId) {
    throw new Error("grantId is required for stampRevokedAt");
  }
  const { baseUrl, authKey } = getSupabaseClientConfig();
  const isoDate = revokedAt || new Date().toISOString();
  const response = await fetch(
    `${baseUrl}/rest/v1/grants?grant_id=eq.${encodeURIComponent(safeGrantId)}&revoked_at=is.null`,
    {
      method: "PATCH",
      headers: buildSupabaseHeaders(authKey, {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({ revoked_at: isoDate })
    }
  );
  if (!response.ok) {
    const payload = await parseSupabaseResponse(response);
    throw new Error(
      `Supabase stampRevokedAt failed (${response.status}): ${typeof payload === "string" ? payload : JSON.stringify(payload)}`
    );
  }
  return { ok: true };
}

async function updateGrantHaveLink(grantId, haveLink) {
  const safeGrantId = sanitizeText(grantId);
  if (!safeGrantId) {
    throw new Error("grantId is required for updateGrantHaveLink");
  }
  const { baseUrl, authKey } = getSupabaseClientConfig();
  const nowIso = new Date().toISOString();
  const response = await fetch(
    `${baseUrl}/rest/v1/grants?grant_id=eq.${encodeURIComponent(safeGrantId)}&deleted_at=is.null`,
    {
      method: "PATCH",
      headers: buildSupabaseHeaders(authKey, {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({ have_link: haveLink, last_checked_at: nowIso })
    }
  );
  if (!response.ok) {
    const payload = await parseSupabaseResponse(response);
    throw new Error(
      `Supabase updateGrantHaveLink failed (${response.status}): ${typeof payload === "string" ? payload : JSON.stringify(payload)}`
    );
  }
  return { ok: true };
}

async function updateGrantTag(grantId, tag) {
  const safeGrantId = sanitizeText(grantId);
  if (!safeGrantId) {
    throw new Error("grantId is required for updateGrantTag");
  }
  const { baseUrl, authKey } = getSupabaseClientConfig();
  const response = await fetch(
    `${baseUrl}/rest/v1/grants?grant_id=eq.${encodeURIComponent(safeGrantId)}&deleted_at=is.null&tag=is.null`,
    {
      method: "PATCH",
      headers: buildSupabaseHeaders(authKey, {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({ tag: sanitizeText(tag) || null })
    }
  );
  if (!response.ok) {
    const payload = await parseSupabaseResponse(response);
    throw new Error(
      `Supabase updateGrantTag failed (${response.status}): ${typeof payload === "string" ? payload : JSON.stringify(payload)}`
    );
  }
  return { ok: true };
}
