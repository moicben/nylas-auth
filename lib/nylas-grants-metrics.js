...
async function countAuthenticatedValidGrants({ apiUrl, apiKey }) {
  const scan = await scanNylasGrantsWithAuth({ apiUrl, apiKey });
  return {
    authenticatedValidGrantCount: scan.authenticatedValidGrantCount,
    totalGrants: scan.totalGrants,
    validGrants: scan.validGrants
  };
}

async function scanNylasGrantsWithAuth({ apiUrl, apiKey }) {
  ...
  const normalizedGrants = rawGrants.map((grant) => ({
    ...
    isValid: isValidGrantStatus(grant),
    isAuthenticated: null,
    createdAt: grant?.created_at || grant?.createdAt || grant?.created_at_ts || grant?.created || null
  }));
  const byId = new Map(normalizedGrants.map((grant) => [grant.id, grant]));
  ...
  for (const grant of validGrants) {
    ...
    try {
      ...
      if (target) {
        target.isAuthenticated = authenticated;
      }
      if (authenticated) {
